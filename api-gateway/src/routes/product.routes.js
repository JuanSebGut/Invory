...
async function proxyToProductService(req, res, productServiceUrl, path, method, fetchImpl) {
  ...
  const response = await fetchImpl(buildProxyUrl(productServiceUrl, path, req.query), {
    method,
    headers,
...
function createProductRoutes({ productServiceUrl, authMiddleware, fetchImpl = fetch }) {
  const router = Router();
