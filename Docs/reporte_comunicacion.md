...
*   **Eventos y Webhooks (Asíncrono/Desacoplado):** Usado principalmente para el sistema de Auditoría. Servicios como `auth-service`, `user-service`, `client-service` y `inventory-service` utilizan variables de entorno como `MS09_AUDIT_WEBHOOK_URL`. Cuando ocurre una acción crítica (ej. login, creación de producto, movimiento de stock), el servicio responsable dispara una petición POST (Webhook) de "fire-and-forget" al `audit-service` para que este registre el evento en el historial sin bloquear la operación principal.
