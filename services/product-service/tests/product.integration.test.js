...
test('POST /api/products rechaza cÃ³digo de barras duplicado EAN-13', async () => {
...
test('POST /api/products valida que la categorÃ­a exista y estÃ© activa', async () => {
  const { app } = buildTestContext();

  const inactiveResponse = await request(app).post('/api/products').send({
    nombre: 'CategorÃ­a Inactiva',
...
test('GET /api/products pagina y filtra por nombre, categorÃ­a y cÃ³digo', async () => {
  const { app } = buildTestContext();
...
test('PUT /api/products/:id actualiza parcialmente y bloquea cambio de barcode', async () => {
  ...
  const okResponse = await request(app).put('/api/products/2').send({
    nombre: 'Papas ClÃ¡sicas XL',
    precio_venta: 14,
    stock_minimo: 2,
  });

  assert.equal(okResponse.status, 200);
  assert.equal(okResponse.body.data.nombre, 'Papas ClÃ¡sicas XL');
...
test('DELETE /api/products/:id realiza borrado lÃ³gico y lo excluye del listado', async () => {
  const { app, repository } = buildTestContext();
